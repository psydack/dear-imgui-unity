...
namespace ImGuiNET.Unity
{
	internal class SpriteInfo
	{
		...
		public NumericsV2f size;
		public NumericsV2f uv0, uv1;
	}
	...
	internal class TextureManager
	{
		...
		private readonly Dictionary<Texture, int> _textureIds = new Dictionary<Texture, int>();
		private readonly Dictionary<Sprite, SpriteInfo> _spriteData = new Dictionary<Sprite, SpriteInfo>();
		private readonly HashSet<IntPtr> _allocatedGlyphRangeArrays = new HashSet<IntPtr>(IntPtrEqualityComparer.Instance);
		...
		public SpriteInfo GetSpriteInfo(Sprite sprite)
		{
			if (!_spriteData.TryGetValue(sprite, out SpriteInfo sprInfo))
			{
				...
				_spriteData[sprite] = sprInfo = new SpriteInfo
				{
					...
					size = Helper.V2f(sprite.rect.size),
					uv0 = new NumericsV2f(uvs[0].x, 1f - uvs[0].y),
					uv1 = new NumericsV2f(uvs[1].x, 1f - uvs[1].y),
				};
			}
			return sprInfo;
		}
		...
		private unsafe IntPtr AllocateGlyphRangeArray(in FontConfig fontConfig)
		{
			...
			ushort* ranges = (ushort*)Util.Allocate(byteCount);
		...
		public void BuildFontAtlas(ImGuiIOPtr io, in FontAtlasConfigAsset settings)
		{
			...
			{
				string fontPath = System.IO.Path.Combine(Application.streamingAssetsPath, fontDefinition.FontPath);
				...
				ImFontConfigPtr fontConfigPtr = new ImFontConfigPtr(ref fontConfig);
		...
		private unsafe Texture2D CreateAtlasTexture(ImFontAtlasPtr atlas)
		{
			...
			Texture2D atlasTexture = new Texture2D(width, height, TextureFormat.RGBA32, false, false) { filterMode = FilterMode.Point };
